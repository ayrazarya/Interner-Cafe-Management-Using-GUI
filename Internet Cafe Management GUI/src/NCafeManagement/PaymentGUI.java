...
public class PaymentGUI extends javax.swing.JFrame {
    ...
    public PaymentGUI(String username, String password, Connection connection) {
        initComponents();
        setTitle("Payment");
        this.setLocationRelativeTo(null);
         Username = username;
...
private String getAdminType(String username) {
    ...
    private void BackButtonMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_BackButtonMouseClicked
        PCManagerGUI pcManagerGUI = new PCManagerGUI(loggedInUsername, Password, Con);
    ...
    if (namaPelanggan != null && !namaPelanggan.isEmpty()) {
        // Memvalidasi bahwa nama pelanggan hanya mengandung huruf
        if (namaPelanggan.matches("[a-zA-Z]+")) {
            try {    
                // Meminta input jumlah pembayaran
                String amountInput = JOptionPane.showInputDialog(null, "Enter the amount");
                
                // Memvalidasi input jumlah pembayaran
                if (amountInput != null && !amountInput.isEmpty() && amountInput.matches("\\d*\\.?\\d+")) {
                    amount = Double.parseDouble(amountInput);
                    
                    // Melakukan penambahan transaksi ke database
                    PreparedStatement addTransactionStatement = Con.prepareStatement("INSERT INTO tb_pembayaran (nama_pelanggan, harga_pembayaran, type_pembayaran) VALUES (?, ?, ?)");
                    ...
                    if (rowsAffected > 0) {
                        ...
                        QRIS qris = new QRIS(Username, Password, Con);
                    ...
                    JOptionPane.showMessageDialog(null, "Invalid amount input. Please enter a valid number.", "Error", JOptionPane.ERROR_MESSAGE);
                }
            } catch (SQLException e) {
                e.printStackTrace();
            ...
            JOptionPane.showMessageDialog(null, "Invalid customer name. Please enter letters only.", "Error", JOptionPane.ERROR_MESSAGE);
        }
    } else {
        JOptionPane.showMessageDialog(null, "No customer name provided.", "Error", JOptionPane.ERROR_MESSAGE);
    }
    }
        ...
        try {
            ...
            if (amountInput != null && !amountInput.isEmpty() && amountInput.matches("\\d*\\.?\\d+")) {
                ...
                if (rowsAffected > 0) {
                    ...
                    JOptionPane.showMessageDialog(null, "Failed to process transaction.", "Error", JOptionPane.ERROR_MESSAGE);
                }
            } else {
                JOptionPane.showMessageDialog(null, "Invalid amount input. Please enter a valid number.", "Error", JOptionPane.ERROR_MESSAGE);
            }
